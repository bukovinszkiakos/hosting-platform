...
#   * AWS/IAM layer      — the role may only call eks:DescribeCluster, which is all
#     `aws eks update-kubeconfig` needs. Every other action the workflow performs
#     is `kubectl`, which IAM does not authorize.
...
# Least privilege at the IAM layer: the workflow's only AWS API call is
# `aws eks update-kubeconfig`, which requires eks:DescribeCluster on this cluster.
